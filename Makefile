...
# build the wasm and js sqlite files, then copy the fiddle HTML and js into
# dist. Finally, remove the amalgamation + extra code file
$(TARGET_SQLJS): dist $(TARGET_SQLITE3_EXTRA_C)\
	...
	cp sqlite-wasm/fiddle/*.{js,css,html,wasm} dist/
	rm $(TARGET_SQLITE3_EXTRA_C)
